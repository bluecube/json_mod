...
def simple_test():
    assert_iterables_equal(tokenize.tokenize_iterable('abcd { , [\ntrue "xyz"\n\n\n7 /* this is a comment\n! */ a'),
...
def unexpected_character_test():
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('abc def @'))

def identifier_test():
    data = 'simple'
    assert_iterables_equal(tokenize.tokenize_iterable(data),
...
def invalid_comment_test():
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('a/a'))

    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('a/ * */'))
...
def invalid_quoted_string_test():
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('a "\nb"a'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('a " b'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable(r'a " b \"'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable(r'"\uxyzw"'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable(r'"\u10"'))
    with assert_raises(ValueError):
...
def json_number_test():
    ...
    assert_iterables_equal(tokenize.tokenize_iterable('42'),
                           [('number', tokenize.Position(None, 1, 1), 42)])
    assert_iterables_equal(tokenize.tokenize_iterable('0.2'),
                           [('number', tokenize.Position(None, 1, 1), 0.2)])
    assert_iterables_equal(tokenize.tokenize_iterable('1.2'),
                           [('number', tokenize.Position(None, 1, 1), 1.2)])
    assert_iterables_equal(tokenize.tokenize_iterable('1.25'),
                           [('number', tokenize.Position(None, 1, 1), 1.25)])
    assert_iterables_equal(tokenize.tokenize_iterable('42.5'),
                           [('number', tokenize.Position(None, 1, 1), 42.5)])
    assert_iterables_equal(tokenize.tokenize_iterable('42.52'),
                           [('number', tokenize.Position(None, 1, 1), 42.52)])
    assert_iterables_equal(tokenize.tokenize_iterable('5e0'),
                           [('number', tokenize.Position(None, 1, 1), 5e0)])
    assert_iterables_equal(tokenize.tokenize_iterable('1e1'),
                           [('number', tokenize.Position(None, 1, 1), 1e1)])
    assert_iterables_equal(tokenize.tokenize_iterable('42E+1'),
                           [('number', tokenize.Position(None, 1, 1), 42e+1)])
    assert_iterables_equal(tokenize.tokenize_iterable('1.2e-1'),
                           [('number', tokenize.Position(None, 1, 1), 1.2e-1)])
    assert_iterables_equal(tokenize.tokenize_iterable('2e-10'),
    ...
    assert_iterables_equal(tokenize.tokenize_iterable('0asdf'),
                           [('number', tokenize.Position(None, 1, 1), 0),
                            ('identifier', tokenize.Position(None, 1, 2), 'asdf')])

def ext_number_test():
    assert_iterables_equal(tokenize.tokenize_iterable('0x1'),
                           [('ext_number', tokenize.Position(None, 1, 1), 1)])
    assert_iterables_equal(tokenize.tokenize_iterable('0x10'),
                           [('ext_number', tokenize.Position(None, 1, 1), 16)])
    assert_iterables_equal(tokenize.tokenize_iterable('0b01001'),
                           [('ext_number', tokenize.Position(None, 1, 1), 9)])
    assert_iterables_equal(tokenize.tokenize_iterable('0o21'),
                           [('ext_number', tokenize.Position(None, 1, 1), 17)])

def invalid_number_test():
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('0xzzz'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('0b'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('00'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('05'))
    with assert_raises(ValueError):
    ...
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('1Easdf'))
    with assert_raises(ValueError):
        list(tokenize.tokenize_iterable('1E+asdf'))
...
def file_positions_test():
    fp = io.StringIO("xy\nXY")
    fp.name = "Z"
    assert_iterables_equal(tokenize.positions_file(fp),
...
def string_positions_test():
    assert_iterables_equal(tokenize.positions("xy\nXY"),
                           [('x', tokenize.Position(None, 1, 1)),
                            ('y', tokenize.Position(None, 1, 2)),
                            ('\n', tokenize.Position(None, 1, 3)),
                            ('X', tokenize.Position(None, 2, 1)),
...
def iterable_positions_test():
    ...
    assert_iterables_equal(tokenize.positions(it()),
                           [('x', tokenize.Position(None, 1, 1)),
                            ('y', tokenize.Position(None, 1, 2)),
                            ('\n', tokenize.Position(None, 1, 3)),
                            ('X', tokenize.Position(None, 2, 1)),
